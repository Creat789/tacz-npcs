...
public class BanditEntity extends AbstractScavEntity {
    ...
    public BrainActivityGroup<? extends AbstractScavEntity> getCoreTasks() {
        return BrainActivityGroup.coreTasks(new Behavior[]{
                (new TargetOrRetaliate<BanditEntity>().isAllyIf((e, l) -> l instanceof BanditEntity).alertAlliesWhen((d, e) -> d.getTarget() != null)),
                (new AvoidEntity<BanditEntity>()).avoiding((entity) -> {return entity instanceof Player;}),
                (new LookAtTarget<BanditEntity>()).runFor((entity) -> {return RandomSource.create().nextInt(40, 300);}),
                //(new StrafeTarget<BanditEntity>()).stopStrafingWhen((entity) -> {
